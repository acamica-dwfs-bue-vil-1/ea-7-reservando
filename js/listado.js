...
Listado.prototype.buscarRestaurante = function(id) {
    let restaurantBuscado = this.restaurantes.find((restaurant) => {
        let resultado = restaurant.id === id ? restaurant : 'No se ha encontrado ningún restaurant.' 
        return resultado;
    });
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    let ubicaciones = this.restaurantes.map(elemento => elemento.ubicacion);
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    var ubicacionesFiltradas = eliminarRepetidos(ubicaciones);
...
Listado.prototype.obtenerRubros = function() {
    let rubros = this.restaurantes.map(elemento => elemento.rubro);    
...
Listado.prototype.obtenerHorarios = function() {
    ...
    let arregloHorarios = this.restaurantes.map(elemento => elemento.horarios);
    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horarios = [];
    arregloHorarios.forEach(function(elemento) {
        elemento.forEach(function(horario) {
